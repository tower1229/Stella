...
当触发自拍时如果没有明确场景要求，比如只说“发张自拍”，或者只给了一半场景，比如“来张窗边的”“发张在外面的图”，Stella 会尝试用家族插件 [stella-timeline-plugin](https://www.npmjs.com/package/stella-timeline-plugin) 做上下文补全：
...
- 半句也能接上：就算你只说“来张现在的”“发张在窗边的”“来张外面的”，timeline 也能把地点、活动、情绪、穿着、时段这些关键现实锚点补齐，不用你自己把 prompt 写得很累。
...
- 不抢用户主意：如果你已经明确指定了地点、穿着、风格，timeline 不会硬把它改掉，而是优先补齐那些你没说但画面里应该有的真实细节。
- 镜头变化：连续性场景会更偏自拍模式，状态变化的场景会更适合镜像模式或游客模式；同一个人设，在不同现实语境下会自然切换最像真的拍法。

当然了，没有安装 stella-timeline-plugin 也不影响正常使用，只是无法实现这些联动。

关于可选 timeline 增强的详细消费与 prompt 组装规则，见 [docs/timeline-integration.md](docs/timeline-integration.md)。
